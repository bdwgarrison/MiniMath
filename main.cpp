...
@example Interpolation (manual entry of file)
    ...
    std::ifstream infile;
    infile.open("/Users/bren/MiniMath/newtondata.txt");
...
@example Solve Matrix
    ...
    std::ifstream infile;
    infile.open("/Users/bren/MiniMath/my_data.txt");
...
int main()
{
    ...
    std::ifstream infile;
    infile.open("/J/Work Orders Data/Data-1000's/WO-1000-0321/WO-1000-0321-Thickness.txt");
    if ( infile.is_open() ){

        fnc::matrix my_matrix(infile);

        alg::interp_basicpw interpolated_vals( my_matrix );

        fnc::pwfunction my_pwfunction = interpolated_vals.getsoln();


        std::cout<< my_pwfunction.lobound() << "\n" << my_pwfunction.upbound();
        //alg::differentiate dif( my_pwfunction, 1, 36, 100,15);

        //std::cout<< dif.getsoln();

        //fnc::pwfunction firstDerivative = dif.piece();

        //std::cout<< firstDerivative(16);
